...
from flask import (
    ...
    flash, jsonify, session, make_response, send_file, abort
)
...
def index():
    ...
    if not conn:
        return "Database Connection Failed. Please ensure MySQL is running and railway_db is imported.", 500
...
def register():
    ...
    if request.method == 'POST':
        ...
        if not username or not password or not fullname:
            flash('অনুগ্রহ করে সব প্রয়োজনীয় তথ্য দিন | Please fill all required fields', 'warning')
            return redirect(url_for('register'))

        if len(password) < 6:
            flash('পাসওয়ার্ড কমপক্ষে ৬ অক্ষরের হতে হবে | Password must be at least 6 characters', 'warning')
            return redirect(url_for('register'))
        ...
        try:
            ...
            if email:
                ...
                if cursor.fetchone():
                    flash('ইমেইল ইতিমধ্যে নিবন্ধিত | Email already registered!', 'warning')
                    return redirect(url_for('register'))
            ...
            flash(f'নিবন্ধন সফল! স্বাগতম, {fullname}! | Registration successful!', 'success')
            return redirect(url_for('dashboard'))
...
def profile():
    ...
    try:
        if request.method == 'POST':
            ...
            cursor.execute("SELECT PassengerID FROM passengers WHERE UserID = %s", (user_id,))
            ...
            if passenger:
                cursor.execute("""
                    ...
                    WHERE UserID = %s
                """, (passport if passport else None, dob if dob else None, gender if gender else None, nationality, fullname, user_id))
            else:
                cursor.execute("""
                    INSERT INTO passengers (UserID, PassportNumber, FullName, DateOfBirth, Gender, Nationality)
                    VALUES (%s, %s, %s, %s, %s, %s)
                """, (user_id, passport if passport else None, fullname, dob if dob else None, gender if gender else None, nationality))
            
            conn.commit()
            flash('প্রোফাইল সফলভাবে আপডেট হয়েছে | Profile updated successfully', 'success')
            return redirect(url_for('profile'))
            
        cursor.execute("SELECT Username, Email, Phone, FullName, Role FROM users WHERE UserID = %s", (user_id,))
        user_info = cursor.fetchone()
        
        cursor.execute("SELECT PassportNumber, DateOfBirth, Gender, Nationality FROM passengers WHERE UserID = %s", (user_id,))
...
def dashboard():
    ...
    try:
        ...
        total_spent = sum(float(b['Amount'] or 0) for b in bookings if b['SeatStatus'] == 'Confirmed')
...
def admin_dashboard():
    ...
    try:
        ...
        # Confirmed bookings
        cursor.execute("SELECT COUNT(*) as count FROM bookings WHERE SeatStatus = 'Confirmed'")
        confirmed_bookings = cursor.fetchone()['count']

        # Cancelled bookings
        cursor.execute("SELECT COUNT(*) as count FROM bookings WHERE SeatStatus = 'Cancelled'")
...
def book_ticket(schedule_id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if payment_method in ('Credit Card', 'Debit Card', 'bKash', 'Nagad', 'Rocket'):
            ...
            cursor.execute("""
                ...
                VALUES (%s, %s, %s, %s, %s, %s)
            """, (ticket_id, amount, booking_date, payment_method, payment_status, txn_id))
        ...
        if request.method == 'POST':
            ...
            if method in ('Credit Card', 'Debit Card'):
                ...
                if not is_valid:
                    ...
                    test_cards = ['4111111111111111', '5500000000000004', '4242424242424242']
                ...
                if mobile_number and len(mobile_number) == 11 and mobile_number.isdigit():
                    if mobile_number.startswith(('013', '014', '015', '016', '017', '018', '019')):
                        if pin and len(pin) >= 4:
                            is_valid = True

            if not is_valid:
                flash('ভুল পেমেন্ট তথ্য | Invalid payment details. Please check and try again.', 'danger')
...
def download_ticket(ticket_id):
    ...
    if not HAS_REPORTLAB:
        flash('PDF generation is not available. Install reportlab: pip install reportlab', 'warning')
    ...
    try:
        ...
        c.drawCentredString(width / 2, height - 100, f"PNR: {ticket['pnr_number'] or 'N/A'}")
        ...
        c.drawCentredString(width / 2, y, "This is a computer-generated ticket. No signature required.")
        c.drawCentredString(width / 2, y - 12, "Bangladesh Railway Reservation System | DBMS Course Project 2026")
...
def cancel_ticket(ticket_id):
    ...
    try:
        ...
        conn.commit()
        flash(f'টিকেট বাতিল! ৳{float(refund_amount):.2f} ফেরত দেওয়া হবে | Ticket cancelled! Refund: ৳{float(refund_amount):.2f}', 'success')
...
def view_coaches(train_number):
    ...
    try:
        cursor.execute("SELECT TrainNumber, TrainName, TotalSeats FROM trains WHERE TrainNumber = %s", (train_number,))
...
def api_coaches(train_number):
    ...
    try:
        ...
        """, (train_number,))
        coaches = cursor.fetchall()
        return cached_response({'coaches': coaches, 'train_number': train_number}, max_age=1800)
    finally:
        cursor.close()
        conn.close()
